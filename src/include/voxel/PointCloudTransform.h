...
public:
  PointCloudTransform(uint32_t left, uint32_t top, uint32_t width, uint32_t height, 
					  ...
					  float fPixelDegree1, float fPixelDegree2, float fPixelDegree3);
...
private:
  ...
  Point _normalizedScreenToUnitWorld3(const Point &screen, const Point &normalizedScreen);
  Point _normalizedScreenToUnitWorld2(const Point &screen, const Point &normalizedScreen);
