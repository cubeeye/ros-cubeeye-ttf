...
public:
 ...
 #else 
	inline static bool setSDKPath(const String &path) { return setEnvironmentVariable("TTF_SDK_PATH", path); }
	inline static bool getSDKPath(String &path) { return getEnvironmentVariable("TTF_SDK_PATH", path); }
 #endif
...
bool ConfigurationFile::_getData(const String &fileName, Vector<T> &data)
{
  ...
  //String fileName2 = "C:\\TTF_Viewer\\Analysis\\Calibration Data\\TIOP8241_MRN000000007\\FPPN\\2017-01-10 225814\\MRN000000007_FPPN.bin";
  String fileName2 = fileName;

  if (_dataFiles.find(fileName2) == _dataFiles.end())
  {
    if(_mainConfigurationFile && _parentID >= 0)
    {
	  ...
	  if (parentConfig && parentConfig->_getData<T>(fileName2, data))
        return true;
      else
        loadFromFile = true;
    }
  ...
  if(loadFromFile)
  {
	...
	Vector<ByteType> &d = _dataFiles[fileName2];
